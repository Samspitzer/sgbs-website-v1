    ...
    <div className="bg-dark-950">
        ...
        <div className="relative h-full w-full px-6 md:px-14 lg:px-20 xl:px-24">
          <div className="flex flex-col justify-center min-h-screen lg:h-full pt-24 pb-12 lg:pt-20 lg:pb-0">
            <div className="max-w-3xl">
              ...
              {/* Inline Stats */}
              <div className="mt-8 lg:mt-10 grid grid-cols-2 sm:flex sm:flex-wrap gap-6 lg:gap-8 animate-fade-in-up stagger-3">
                {stats.map((stat) => (
                  <div key={stat.label} className="hero-text-strong">
                    <div className="text-2xl lg:text-3xl font-display font-bold text-accent-400">{stat.value}</div>
                    <div className="text-white/70 text-xs lg:text-sm uppercase tracking-wide">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="min-h-screen lg:h-screen flex flex-col justify-center snap-section bg-dark-900 relative overflow-hidden noise-overlay py-20 lg:py-0">
        <div className="container-custom relative w-full">
          {/* Header + Filters */}
          <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-6 lg:mb-8">
            <div className="animate-fade-in-up">
              <div className="line-accent mb-4" />
              <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-cream-100">
                Project Gallery
              </h2>
            </div>
            <div className="flex flex-wrap items-center gap-2 mt-4 md:mt-0 animate-fade-in-up stagger-1">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 lg:px-5 py-2 text-xs font-semibold tracking-wide uppercase transition-all duration-300 ${
          ...
          {/* Projects Grid */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 animate-fade-in-up stagger-2">
            {filteredProjects.slice(0, 6).map((project) => (
              <article key={project.id} className="group relative overflow-hidden card-hover" style={{ borderRadius: '4px' }}>
                ...
                <div className="absolute inset-0 p-4 lg:p-5 flex flex-col justify-end">
                  <p className="text-accent-400 text-xs font-semibold tracking-wide uppercase mb-1">{project.units}</p>
                  <h3 className="text-lg lg:text-xl font-display font-semibold text-cream-100 mb-0.5">{project.name}</h3>
                  <p className="text-cream-100/70 text-sm">{project.location}</p>
                  {project.description && (
