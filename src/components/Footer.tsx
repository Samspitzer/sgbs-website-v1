...
interface FooterProps { isSnapSection?: boolean; }

export function Footer({ isSnapSection = false }: FooterProps) {
  ...
  return (
    <section id="contact" className={sectionClass} style={!isSnapSection ? { minHeight: '70vh' } : {}}>
      <div className="absolute inset-0 overflow-hidden">
        <img src="/images/backgrounds/cta.jpeg" alt="" className="w-full h-full object-cover" style={{ filter: 'blur(2px)', transform: 'scale(1.05)' }} />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.3) 25%, rgba(0,0,0,0.35) 50%, rgba(0,0,0,0.5) 80%, rgba(0,0,0,0.7) 100%)' }} />
      </div>

      <div className="relative flex flex-col" style={isSnapSection ? { height: '100%' } : { minHeight: '70vh' }}>
        {/* Main content - NO items-center, use explicit padding to clear header */}
        <div className="flex-1 relative">
          <div className="container-custom w-full pt-24 lg:pt-[104px] 2xl:pt-[116px] pb-3">

            <div className="text-center mb-3 lg:mb-4">
              <div className="line-accent mx-auto mb-2" />
              <h2 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-[2rem] xl:text-[2.25rem] 2xl:text-5xl font-bold text-white" style={{ textShadow: '0 2px 4px rgba(0,0,0,1), 0 0 8px rgba(0,0,0,0.8)' }}>
                Let's Start Your Project
              </h2>
              <p className="mt-1.5 lg:mt-2 text-white/80 text-sm lg:text-base max-w-lg mx-auto font-medium" style={{ textShadow: sharp }}>
                Get a quote or ask a question — we respond within 24 hours.
              </p>
            </div>

            <div className="grid lg:grid-cols-3 gap-4 lg:gap-4 xl:gap-5 2xl:gap-8 max-w-5xl mx-auto items-start">
              {/* Contact Info + Map */}
              <div className="lg:col-span-1">
                <h3 className="text-xs lg:text-sm font-bold text-white mb-2.5 lg:mb-3 uppercase tracking-wider" style={{ textShadow: sharp }}>Contact Info</h3>
              ...
              <div className="lg:col-span-2">
                <h3 className="text-xs lg:text-sm font-bold text-white mb-0.5 uppercase tracking-wider" style={{ textShadow: sharp }}>Request a Quote</h3>
                <p className="text-white/60 text-[11px] lg:text-xs mb-2.5 lg:mb-3 font-medium" style={{ textShadow: sharpLight }}>Fill out the form and we'll get back to you within 24 hours.</p>
                {formState === 'success' ? (
                  <div className="text-center py-8">
                    <div className="w-12 h-12 mx-auto bg-accent-500/15 rounded-full flex items-center justify-center mb-3"><CheckCircle className="w-6 h-6 text-accent-400" /></div>
                    <h4 className="text-lg font-bold text-white mb-1.5">Thank You!</h4>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3 md:space-y-2 lg:space-y-2">
                    ...
                    <button type="submit" disabled={formState === 'submitting'} className="w-full py-2 lg:py-2.5 bg-accent-500 text-white text-xs lg:text-sm font-bold tracking-wide uppercase hover:bg-accent-400 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                      {formState === 'submitting' ? (<><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Sending...</>) : (<>Submit Request <Send className="w-4 h-4" /></>)}
                    </button>
                  </form>
                )}
              </div>
        ...
        <footer className="relative">
          ...
          <div className="container-custom py-2.5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <img src="/images/logo.png" alt="S&G Builders Supply Inc." className="h-7 lg:h-8 w-auto" />
                <p className="text-white/60 text-[11px] hidden sm:block font-medium">Your single source for doors, frames, hardware, and molding.</p>
              </div>
              <nav className="flex items-center gap-4">
                <Link to="/about" className="text-white/60 hover:text-accent-400 transition-colors text-[11px] font-medium">About</Link>
                <Link to="/services" className="text-white/60 hover:text-accent-400 transition-colors text-[11px] font-medium">Services</Link>
                <Link to="/projects" className="text-white/60 hover:text-accent-400 transition-colors text-[11px] font-medium">Projects</Link>
          ...
          <div className="relative">
            ...
            <div className="container-custom py-2 flex items-center justify-between text-[11px] text-white/50 font-medium">
